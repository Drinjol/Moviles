...
public partial class EditarPerfil : ContentPage, INotifyPropertyChanged
{
    ...
    private async void GuardarInformacion_Clicked(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                var response = await httpClient.PostAsync(API_LINK.link + "CommunyStoreApi/usuario/actualizarUsuario", new StringContent(usuarioJson, Encoding.UTF8, "application/json"));
